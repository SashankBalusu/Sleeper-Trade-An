...
async function submitLeagueIDAsync() {
  ...
  for (let i of playerPosCopy) {
    if (i != "BN" && i != "FLEX" && i != "DEF") {
      for (let key in ownerPosRank) {
        // username = await httpGetAsync(`https://api.sleeper.app/v1/user/${key}`)
  ...
  let myFunction = function (elements, loc) {
    ...
    for (let i = 1; i < weeksPassed; i++) {
      // let matchupData = await httpGetAsync(`https://api.sleeper.app/v1/league/650072723749421056/matchups/${(i)}`)
